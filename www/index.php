...
<body>
       ...
       <nav class="navbar navbar-dark navbar-expand-md fixed-top">
        ...
        <div class="container">
           ...
           <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item"><a class="nav-link" href="#contact"><span class="fa fa-address-card fa-lg"> </span> Contact us</a>
               <?php
                if ( isset($_SESSION['email']) ) {
                    $stmt=$pdo->prepare("SELECT fname,lname FROM user where email=:xyz");
    ...
    <div class="bgimg">
        ...
        <section id="home">
            	 <a id="DonorButton" href='logout.php'><button class="btn text-white btn-lg " style="background-color:#b0e0e6"><span class="fa fa-sign-in"></span>LOGOUT</button></a>
            ...
            if(isset($_POST['city']) && isset($_POST['bloodgrp'])) 
{
    ...
    ?>
					...
					<div class="text-center text-white headerset">
                        ...
                        <br><br><br><br><br><br>
                        <br><center><a href="register.php"><button class="btn text-white btn-lg" style="background-color:firebrick;">DONATE</button></a></center>
    ...
    <?php
        ...
        $r=$pdo->query('SELECT COUNT(*) FROM add_camp')->fetchColumn();
        if($r!=0)
        {
            echo('<div class="row row-content media-row align-items-center" id="reviews">'."\n");
            echo('<div class="col-12 col-sm-4 col-md-3">'."\n");
            echo('<h2 style="color:purple">FUTURE CAMPS</h2>'."\n");
            echo("</div>"."\n");
            
        
        
            $stmt=$pdo->query("SELECT camp_name,camp_loc,camp_date FROM add_camp");
             while($row=$stmt->fetch(PDO::FETCH_ASSOC))
             {
            ...
            echo('<h2 class="mt-0">'.htmlentities($row['camp_name']). '<span class="badge badge-danger">NEW</span>'.'</h2>');
                 
                 echo('<h5 class="d-none d-sm-block" style="color:crimson;">');
                  echo('<span style="color:black">Camp Location: </span>'.htmlentities($row['camp_loc']));
                 echo('<br>');
                  echo('<span style="color:black">Camp Date: </span>'.htmlentities($row['camp_date']));
                 echo('</h5>');
